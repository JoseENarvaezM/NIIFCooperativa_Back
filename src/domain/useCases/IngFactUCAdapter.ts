...
export class IngFactUCAdapter
  implements FormsUCIntPort<FormIngresosFancturacion>
{
  constructor(
  ...
  async getIDForm(
    stuID: string,
    roomID: string
  ): Promise<FormIngresosFancturacion | null> {
    const formIngresos = await this.ingFactGateway.getIDForm(stuID, roomID);
    if (!formIngresos) {
      this.errorFormatter.errorNotFound(
        `Formulario con stuID ${stuID} y roomID ${roomID} no se encontró.`
      );
    }
    const formIngresosCalculated = await this.ingFactGateway.calculateReference(
      stuID,
      roomID
    );
  ...
  async updateForm(
    stuID: string,
    roomID: string,
    ingFact: FormIngresosFancturacion
  ): Promise<FormIngresosFancturacion> {
